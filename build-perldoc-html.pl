...
foreach my $module_index ('A'..'Z') {
  ...
  $page_data{breadcrumbs} = [ {name=>'Core modules', url=>"index-modules-A.html"} ];
  ...
  foreach my $module (grep {/^$module_index/ && exists($Perldoc::Page::CoreList{$_})} Perldoc::Page::list()) {
    ...
    $module_data{breadcrumb}  = [ {name=>'Core modules', url=>"index-modules-A.html"},
                                  {name=>$module_index, url=>"index-modules-$module_index.html"} ];
